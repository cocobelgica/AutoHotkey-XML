...
class xml
{
	...
	__Get(property) {
		if !ObjHasKey(this, property) { ; Redundant??
			if (property = "file")
				...
				if (property.HasKey("ssn") || property.HasKey("sn")) {
					m := ((xp:=property["sn"])
					  ?  "selectNodes"
					  :  ((xp:=property["ssn"])
					  ?  "selectSingleNode"
					  :  false))
					return m ? this[m](xp) : m
				} else {
					n := [] , i := 0
					for a, b in property
						Mod(A_Index, 2) ; use A_Index in case of non-integer keys
						? m := {sn: "selectNodes", ssn: "selectSingleNode"}[b]
						: n[i+=1] := this[m](b)
					return n.MaxIndex()>1 ? n : n.1
				}
			} else {
	...
	*/
	...
	PARAMETER(s):
	    ...
	    regex   -   The 'name' parameter is treated as a RegEx experssion.
	                Specifying '1' will return the first found match, while
	                '2' will return an array of all the attributes nodes
	                whose 'nodeName' property matched the pattern. By default,
	                this parameter is set ot '0'(disabled).
	USAGE:
	    xmlObj.getAtt("//Node", "name")
	RETURN VALUE:   The string that contains the attribute value.
	*/
	
	getAtt(element, name, regex:=0) {
		e := IsObject(element) ? element : this.selectSingleNode(element)
		if regex {
			n := [] , i := 0
			Loop, % (att := e.attributes).length {
				_a := att.item(A_Index-1)
				if (_a.nodeName ~= name) {
					if (regex == 1)
						return _a
					else if (regex == 2)
